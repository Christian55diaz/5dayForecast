...
function init(){
    ...
    // If cities were fetched from localStorage, update the cities
    if (storedCities !== null) {
        cities = storedCities;
      }
    // Render cities go to the DOM
    renderCities();
}

function storeCities(){
   // Stringify "cities" so that its kept in localstorage in an array
...
function renderCities() {
  ...
  //Only works when the form is submitted
  $("#add-city").on("click", function(event){
      event.preventDefault();

    // Grabs the city from input
    var city = $("#city-input").val().trim();
    
    //if city is submitted blank return early
    if (city === "") {
        return;
    }
  ...
  function getResponseWeather(cityName){
    ...
    $.ajax({
      ...
      // Creating a new table row element for the weather description for the days forecast
